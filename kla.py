...
def getKLACmd(tmp_dir=TMP_EXTRACTION_DIR,
    ...
    klablcmd = '{} -n {} -s {} --user {} --pass {} -as {} -cr {} -crs {} -bn {} -wr transactsql.parser.valid.list={} {}'.format(agent, appname, sourcedir, user, password, scope, changerequest, status, branch, dbtype, advanced)
    return klablcmd
...
def getBLAnalysisResultsURL(a_c, kla_user=PARAM_KLA_USERNAME, kla_password=PARAM_KLA_PASSWORD):
    ...
    if not PARAM_KLA_KIUWANBASEURL:
...
if rc==0:
  ...
  print("::set-output name=message::{}".format('Delivery analysis successful.'))
elif rc == 1:
